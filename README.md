...
- `sensor_simulation` / `sensor_simulation.exe`: Local binary after build (not committed; see `.gitignore`).
- `sensor_log.txt`: Append-only log created when you run the program (not committed; see `.gitignore`).
- `.gitignore`: Keeps logs and build artifacts out of Git.
